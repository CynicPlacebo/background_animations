...
var G = {
  ...
  jitterFactor:1000, //Number of decimals to round Jitter to (e.g. 100 = 2 decimals)
  jitterMax:0.2,
  jitterMin:-0.2,
...
function init() {
  ...
  let jitterRange = G.jitterMax - G.jitterMin;
  for (e of bounceImgs.childNodes) {
    if (e.tagName && e.tagName.toUpperCase() == 'IMG') {
      ...
      let jitter = rand + G.jitterMin;
      jitter = Math.round(G.jitterFactor * jitter) / G.jitterFactor; //3 decimals
      console.log("   - Speed Jitter: " + jitter);
      G.img.push(new BounceImg(e, jitter));
    }
  }
